    ...
    <div className="space-y-10">
      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
        ...
        <div className="flex gap-2 sm:gap-3">
          <select className="bg-card-bg border border-white/5 rounded-[16px] sm:rounded-[20px] px-4 sm:px-6 py-3 sm:py-4 text-xs font-bold text-text-muted outline-none focus:border-primary/30 transition-all shadow-xl appearance-none flex-1 sm:flex-none min-w-[120px] sm:min-w-[160px]">
            <option>Todos</option>
            <option>Español</option>
            <option>Inglés</option>
            <option>Portugués</option>
          </select>
          <button className="flex items-center justify-center gap-2 px-4 sm:px-6 py-3 sm:py-4 bg-card-bg border border-white/5 rounded-[16px] sm:rounded-[20px] text-xs font-bold text-text-muted hover:text-white-custom transition-all shadow-xl">
            <Filter size={16} />
        ...
        {conversations.length > 0 ? conversations.map((conv) => (
          <div key={conv.id} className="bg-card-bg rounded-[24px] sm:rounded-[40px] border border-white/5 p-4 sm:p-6 lg:p-8 shadow-2xl relative overflow-hidden group hover:border-primary/30 transition-all duration-500">
            ...
            <div className="space-y-3 sm:space-y-4 mb-4 sm:mb-8 relative z-10">
              ...
              <div className="p-3 sm:p-5 rounded-[20px] sm:rounded-[24px] bg-background/50 border border-white/5 italic">
      ...
      <div className="flex justify-center pt-6 sm:pt-10">
        <button className="px-6 sm:px-10 py-3 sm:py-4 rounded-xl sm:rounded-2xl bg-white/5 border border-white/10 text-[10px] font-black uppercase tracking-[0.2em] sm:tracking-[0.3em] text-text-muted hover:text-white-custom transition-all">
          Cargar Más Sesiones
        </button>
      </div>
    </div>
