...
export default function FAQPage() {
  ...
  return (
    <main className="min-h-screen py-24 px-4 md:py-32 md:px-8 bg-background text-text-main selection:bg-primary/30">
      <JsonLd data={faqData} />
      ...
      <div className="max-w-5xl mx-auto space-y-24">
        ...
        <div className="text-center space-y-6">
          ...
          <p className="text-text-muted text-sm font-medium max-w-2xl mx-auto opacity-70">
          ...
          {CATEGORIES.map((category, idx) => (
            <section key={idx} className="space-y-12">
              <div className="flex items-center gap-4 border-b border-white/5 pb-6">
                ...
                <h2 className="text-2xl font-black text-white-custom tracking-tight italic uppercase">{category.title}</h2>
              </div>
...
function TrustItem({ icon, text }: { icon: React.ReactNode, text: string }) {
  return (
    <div className="flex items-center gap-2 font-black text-[9px] uppercase tracking-[0.4em] text-text-muted">
      {icon}
      {text}
    </div>
  );
